...
from espnet.MyNet.modules.repos_attention import RelPositionalEmbedding
from espnet.MyNet.modules.repos_attention import RelPartialLearnableDecoderLayer
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, n_head, d_model, d_head, pos_ff,
                 dropout, ext_len, mem_len, dropatt, pre_lnorm, tgt_len=None,future_len=0):
    ...
    def forward(self, x, masks):
        ...
        if self.future_len !=0:
            pred_hid = hidden[-(tgt_len+self.future_len):-self.future_len]
        else:
            pred_hid = hidden[-tgt_len:]
        print(self.mems.size())
